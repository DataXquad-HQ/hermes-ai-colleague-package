...
If Leo is present in a group but not responding to `@` mentions, the Hermes gateway group policy is likely unset. See `references/feishu-gateway-group-policy.md` for the fix (`default_group_policy: open`, `require_mention: true` in `config.yaml`) and gateway restart instructions. Cannot restart from inside the gateway — use a separate shell.
